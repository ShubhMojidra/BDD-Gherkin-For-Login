...
const autorelease_pool_1 = require("./autorelease-pool");
/** @type {any} */
let cv;
/**
 * @typedef Region
...
function calculateMatchedRect(matchedPoints) {
    ...
    const firstPoint = /** @type {Point} */ (lodash_1.default.head(pointsSortedByDistance));
    const lastPoint = /** @type {Point} */ (lodash_1.default.last(pointsSortedByDistance));
    const topLeftPoint = {
...
function highlightRegion(mat, region) {
    ...
    const pool = new autorelease_pool_1.OpenCvAutoreleasePool();
    try {
        // highlight in red
...
async function getImagesMatches(img1Data, img2Data, options = {}) {
    ...
    try {
        ...
        if (!lodash_1.default.includes(lodash_1.default.keys(AVAILABLE_DETECTORS), detectorName)) {
            throw new Error(`'${detectorName}' detector is unknown. ` +
                `Only ${JSON.stringify(lodash_1.default.keys(AVAILABLE_DETECTORS))} detectors are supported.`);
        }
        if (!lodash_1.default.includes(lodash_1.default.keys(AVAILABLE_MATCHING_FUNCTIONS), matchFunc)) {
            throw new Error(`'${matchFunc}' matching function is unknown. ` +
                `Only ${JSON.stringify(lodash_1.default.keys(AVAILABLE_MATCHING_FUNCTIONS))} matching functions are supported.`);
        }
        const detector = pool.add(new cv[AVAILABLE_DETECTORS[detectorName]]());
        const [img1, img2] = (await bluebird_1.default.all([cvMatFromImage(img1Data), cvMatFromImage(img2Data)]))
            .map((x) => pool.add(x));
        ...
        const hasGoodMatchesFactor = lodash_1.default.isFunction(goodMatchesFactor) ||
            (lodash_1.default.isNumber(goodMatchesFactor) && !lodash_1.default.isNaN(goodMatchesFactor));
        if (hasGoodMatchesFactor) {
            if (lodash_1.default.isFunction(goodMatchesFactor)) {
                const distances = matches.map((match) => match.distance);
                const minDistance = lodash_1.default.min(distances);
                const maxDistance = lodash_1.default.max(distances);
...
async function getImagesSimilarity(img1Data, img2Data, options = {}) {
    ...
    const pool = new autorelease_pool_1.OpenCvAutoreleasePool();
    try {
        const [template, reference] = (await bluebird_1.default.all([cvMatFromImage(img1Data), cvMatFromImage(img2Data)]))
            .map((x) => pool.add(x));
...
async function getImageOccurrence(fullImgData, partialImgData, options = {}) {
    await initOpenCv();
    const { visualize = false, threshold = DEFAULT_MATCH_THRESHOLD, multiple = false, matchNeighbourThreshold = MATCH_NEIGHBOUR_THRESHOLD, method = DEFAULT_MATCHING_METHOD, } = options;
    const pool = new autorelease_pool_1.OpenCvAutoreleasePool();
    try {
        const [fullImg, partialImg] = await bluebird_1.default.all([cvMatFromImage(fullImgData), cvMatFromImage(partialImgData)])
            .map((x) => pool.add(x));
        const matched = pool.add(new cv.Mat());
        const results = [];
        ...
        if (visualize) {
            ...
            let restPngBuffers = [];
            [visualization, ...restPngBuffers] = await bluebird_1.default.all([cvMatToPng(fullHighlightedImage), ...visualizePromises]);
            for (const [result, pngBuffer] of lodash_1.default.zip(results, restPngBuffers)) {
...
async function cvMatToPng(mat) {
    return await (0, sharp_1.default)(buffer_1.Buffer.from(mat.data), {
        raw: {
            width: mat.cols,
            height: mat.rows,
            channels: 4,
...
async function cvMatFromImage(img) {
    const { data, info } = await (0, sharp_1.default)(img)
        .ensureAlpha()
        .raw()
